...
export class TreeService {
  ...
  private isSnowOnSubject$ = new BehaviorSubject<boolean>(false);
  public isSnowOn$ = this.isSnowOnSubject$.asObservable();
  private isAudioOnSubject$ = new BehaviorSubject<boolean>(false);
  ...
  private audioPlayer = new Audio('assets/audio/audio.mp3');

  constructor() {
    this.isAudioOnSubject$.subscribe((value) => {
      if (value) {
  ...
  onSwitcherGarland(valueSwitcher: boolean) {
  ...
  audioSwitcher() {
    const currentValue = this.isAudioOnSubject$.getValue();
    this.isAudioOnSubject$.next(!currentValue);
  }

  snowSwitcher() {
    const currentValue = this.isSnowOnSubject$.getValue();
    this.isSnowOnSubject$.next(!currentValue);
  }
}
